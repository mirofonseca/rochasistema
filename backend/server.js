...
app.post('/api/clientes', auth, (req, res) => {
  const { nome, cpf, tel, email, endereco, cidade, cnh, cat_cnh, obs } = req.body;
  if (!nome || !tel) return res.status(400).json({ error: 'Nome e telefone obrigatórios' });
  const id = uid();
  run(`INSERT INTO clientes (id,nome,cpf,tel,email,endereco,cidade,cnh,cat_cnh,obs) VALUES (?,?,?,?,?,?,?,?,?,?)`,
    [id,nome,cpf||null,tel,email||null,endereco||null,cidade||null,cnh||null,cat_cnh||null,obs||null]);
...
app.put('/api/clientes/:id', auth, (req, res) => {
  ...
  const { nome, cpf, tel, email, endereco, cidade, cnh, cat_cnh, obs } = req.body;
  run(`UPDATE clientes SET nome=?,cpf=?,tel=?,email=?,endereco=?,cidade=?,cnh=?,cat_cnh=?,obs=? WHERE id=?`,
    [nome||c.nome,cpf||null,tel||c.tel,email||null,endereco||null,cidade||null,cnh||null,cat_cnh||null,obs||null,req.params.id]);
